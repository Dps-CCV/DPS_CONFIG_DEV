...
class NukeCameraPublishPlugin(HookBaseClass):
    """
    ...
    def icon(self):
        ...
        return os.path.join(self.disk_location, "icons", "camera.png")
    ...
    def name(self):
        ...
        """
        return "Publish Camera to Shotgun"
    ...
    def settings(self):
        ...
        plugin_settings = super(NukeCameraPublishPlugin, self).settings or {}
        nuke_camera_publish_settings = {
            "Publish Template": {
                "type": "template",
                "default": None,
        ...
        plugin_settings.update(nuke_camera_publish_settings)
    ...
    def publish(self, settings, item):
        ...
        for dep in publish_dependencies_paths:
            pathSplit = dep.split("/")
            goodPath = os.path.join(*pathSplit[1:]).replace(os.sep, "/")
            depID = engine.shotgun.find_one("PublishedFile", [["path_cache", "is", goodPath]], ["id"])
            if depID:
                publish_dependencies_ids.append(depID["id"])
