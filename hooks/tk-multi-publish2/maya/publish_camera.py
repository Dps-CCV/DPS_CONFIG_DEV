...
class MayaCameraPublishPlugin(HookBaseClass):
    ...
    def publish(self, settings, item):
        ...
        item.properties["publish_root"]

        # ensure the publish folder exists:
        publish_folder = os.path.dirname(publish_path)
        self.parent.ensure_folder_exists(publish_folder)

        #PDL: do the publishes
        self._publish_fbx(settings, item)
        self._publish_alembic(settings, item)
    ...
    def _publish_fbx(self, settings, item):
        """
        This method publishes the selected camera as fbx.
        """
        
        publish_path = item.properties["publish_root"] + '.fbx'
    ...
    def _publish_alembic(self, settings, item):
        """
        This method publishes the selected camera as alembic.
        """
        
        publish_path = item.properties["publish_root"] + '.abc'
        ...
        if start_frame and end_frame:
            alembic_args.append("-frameRange %d %d" % (start_frame, end_frame))
