...
class MayaActions(HookBaseClass):
    ...
    def generate_actions(self, sg_publish_data, actions, ui_area):
        ...
        if "Alembic_Import/Merge" in actions:
            action_instances.append(
                {
                    "name": "Alembic_Import/Merge",
                    "params": None,
                    "caption": "Import/Merge alembic into selected Node",
                    "description": "This will import the alembic and merge it into the current selection.",
                }
            )
        ...
        if "standIn" in actions:
            action_instances.append(
                {
                    "name": "standIn",
                    "params": None,
                    "caption": "Create aiStandIn",
                    "description": "Creates an aiStandIn for the selected item..",
                }
            )
            ...
            action_instances.append(
                {
                    "name": "volume",
                    "params": None,
                    "caption": "Create aiVolume",
                    "description": "Creates an aiVolume for the selected item..",
                }
            )
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        if name == "Alembic_Import/Merge":
            self.import_merge_alembic(path, sg_publish_data)
    ...
    def _create_reference_alembic(self, path, sg_publish_data):
        """
        ...
        # Setup shading networks if needed
        #reference_node = cmds.referenceQuery(path, referenceNode=True)
        #self._hookup_shaders(reference_node)
    ...
    def _create_reference(self, path, sg_publish_data):
        ...
        if con.entity['type'] == "Asset":
            ...
            namespace = asset_name
        # if sg_publish_data.get("entity").get("type") == 'Asset':
        #     namespace = asset_name
        # else:
        #     namespace = ":"
    ...
    def import_merge_alembic(self, path, sg_publish_data):
        """
        Create a reference with the same settings Maya would use
        if you used the create settings dialog.
        ...
        selnodes = cmds.ls(sl=True, long=True)
        sel = ''
        if len(selnodes)> 1:
            cmds.error("Selecciona un solo nodo en el que importar el alembic")
            raise Exception("Selecciona un solo nodo en el que importar - '%s'" % path)
        else:
            sel = selnodes[0]
            # import Merge alembic
            cmds.AbcImport(path, mode="import", connect = str(sel))
    ...
    def _create_image_plane(self, path, sg_publish_data):
        ...
        (img_plane, img_plane_shape) = cmds.imagePlane(camera=str(selCam[0]),
    ...
    def _create_standIn(self, path, sg_publish_data):
        """
        Create an aiStandIn node for ass files

        :param path: Path to file.
        :param sg_publish_data: Shotgun data dictionary with all the standard
            publish fields.
        :returns: The newly created standIn node
        """

        node = cmds.createNode('aiStandIn')
        cmds.setAttr('{}.dso'.format(node), path, type='string')
        cmds.setAttr('{}.useFrameExtension'.format(node), 1)

    def _create_volume(self, path, sg_publish_data):
        """
        Create an aiVolume node for vdb files
    ...
    def _hookup_shaders(self, reference_node):
        """
